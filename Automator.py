...
Label24 = Label(root, text = "Salesforce Autofill").grid(row=13, column=5)
Label25 = Label(root, text = "Confirmation Email").grid(row=14, column = 5)
Label26 = Label(root, text ="CoStar").grid(row=2, column=5)
...
costar = Checkbutton(root, text="", variable=c17).grid(row=2, column = 6, sticky=W)
...
def Master_Function():
   ...
   "safebrowsing.enabled": True
    ...
    if c17.get():
        ...
        time.sleep(1.5)
        chrome_browser.find_element_by_id("username").send_keys("Joe Ma")
        chrome_browser.find_element_by_id("password").send_keys("Joe Ma")
